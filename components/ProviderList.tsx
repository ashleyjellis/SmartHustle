...
export default function ProviderList({ providers }: { providers: Provider[] }) {
  ...
  const [sortDesc, setSortDesc] = useState(true) // true = highest rating first

  const filtered = providers
    .filter(FILTERS[activeIdx].match)
    .sort((a, b) => {
      const ra = a.rating ?? 0
      const rb = b.rating ?? 0
    ...
    <div className="space-y-6">
      ...
      <div id="top-accounts" className="bg-surface-container-lowest border border-outline-variant rounded-xl p-4 flex flex-wrap items-center justify-between gap-4">
        ...
        >
          ...
          <span className="font-bold text-label-sm md:text-body-md">
            Rating: {sortDesc ? 'Highest first' : 'Lowest first'}
          </span>
          <svg
